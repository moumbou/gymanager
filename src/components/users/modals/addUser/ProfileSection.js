...
import { selectCode } from "../../../../features/subs/codeSlice";

const { ipcRenderer } = window.require("electron");

function ProfileSection({ register, errors, setVerification }) {
  const [correctCode, setCode] = useState(false);
  const [input, setInput] = useState("");
  const getCode = useSelector(selectCode);

  const verifyCode = (e) => {
    const value = e.target.value;
    if (value.length >= 5) {
      ipcRenderer.send(FunctionsDB.VERIFY_CODE, value);
      setInput(value);
    } else {
      setCode(false);
    ...
    if (!value.length) setCode(true);
  };

  useEffect(() => {
    if (getCode !== null) {
      setCode(getCode);
      setVerification(getCode);
    }
    if (!input.length) setVerification(true);
  }, [getCode, input, setVerification]);

  return (
    <>
      <h3>profile section</h3>

      <SubPicture register={register} />
